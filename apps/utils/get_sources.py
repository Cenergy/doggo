...
def get_source(sourcename):
    query_sql="select * from resources_sourcescore where sourcename like '%{abc}%'".format(abc=sourcename)
...
def get_source_by_id(source_id):
    query_sql = "select * from resources_sourcescore where id={}".format(source_id)
    all_data = pd.read_sql(query_sql, connection)
    data_dict = all_data.to_dict(orient='index')
    return data_dict
