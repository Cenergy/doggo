    ...
    var LocationField = {
      ...
      render: function () {
        ...
        if (!this.providers.test(this.options.provider)) {
          this.error(
            "render failed, invalid map provider: " + this.options.provider
          );
          return;
        }
        ...
        this.loadAll(function () {
          ...
          console.log("rdapp - mapOptions", mapOptions);
      ...
      search: function (map, marker, address) {
        if (this.options.searchProvider === "google") {
          ...
          var url =
            "//geocode-maps.yandex.ru/1.x/?format=json&geocode=" + address;

          if (
            typeof this.options.providerOptions.yandex.apiKey !== "undefined"
          ) {
          ...
          var url =
            "https://api-adresse.data.gouv.fr/search/?limit=1&q=" + address;
          ...
          var url =
            "//nominatim.openstreetmap.org/search/?format=json&q=" + address;
        ...
        this.load.common(function () {
          var mapProvider = self.options.provider,
            onLoadMapProvider = function () {
              var searchProvider =
                  self.options.searchProvider + "SearchProvider",
              ...
              if (self.load[searchProvider] != undefined) {
                self.load[searchProvider](
                  self.options.providerOptions[self.options.searchProvider] ||
                    {},
      ...
      load: {
        google: function (options, onload) {
          ...
          var js = [url, this.path + "/leaflet-google.js"];
        ...
        googleSearchProvider: function (options, onload) {
          ...
          var js = [url, this.path + "/l.geosearch.provider.google.js"];
      ...
      _getMap: function (mapOptions) {
        ...
        var map = new ol.Map({
          ...
          view: new ol.View({
            center: ol.proj.fromLonLat(mapOptions.center),
            maxZoom: 18,
            zoom: 7,
          }),
      ...
      _getMarker: function (map, center) {
        ...
        console.log("rdapp - self", self.options.path);
        ...
        MarkerIcon.setStyle(
          new ol.style.Style({
            image: new ol.style.Icon({
              ...
              anchorYUnits: "pixels",
              src: self.options.path + "/images/marker-icon.png",
              scale: 0.4,
            }),
          })
        );
        ...
        console.log("rdapp - MapLayer", MapLayer);
        // Add marker to layer
      ...
      _watchBasedFields: function (map, marker) {
        ...
        basedFields.each(function () {
          ...
          if (el.is("select")) el.change(onchange);
          else el.keyup(onchange);
        });
      },
